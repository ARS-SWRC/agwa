...
def initialize_workspace(workspace, delineation_name, discretization_name, model, methodology, threshold,
                         internal_pour_points):
...
def discretize(workspace, discretization_name, save_intermediate_outputs):
    arcpy.env.workspace = workspace
    ...
    # flow length downstream is used in element parameterization for element-based flow length calculations
    ...
    # FIXME: intermediate outputs will be saved regardless of conditional if an error encountered is encountered first
    # TODO: Related to prior comment, address handling and reporting of errors
    ...
    arcpy.SetParameter(8, discretization_feature_class)
    arcpy.SetParameter(9, streams_feature_class)
