...
class PlotHydrograph(object):
    ...
    def getParameterInfo(self):
        ...
        param3 = arcpy.Parameter(displayName="Plot Hillslope Element(s)",  
                                    name="Plot_Hillslope_Elements",
                                    datatype="GPBoolean",
        ...
        # Channel parameters
        param9 = arcpy.Parameter(displayName="Plot Channel Element(s)",
                            name="Plot_Channel_Elements",
                            datatype="GPBoolean",
                            parameterType="Optional",
                            direction="Input")
        param9.enabled = True
        ...
        param18 = arcpy.Parameter(displayName="Plot Same Type of Element(s) in One Graph",
                                    name= "Multiple_Elements",
                                    datatype="GPBoolean",
                                    parameterType="Optional",
                                    direction="Input")
        param18.category = "Display Options"
        param19 = arcpy.Parameter(displayName="Auto-Display Graph(s) on Screen",
        ...
        param20 = arcpy.Parameter(displayName="Show Graph File(s) in Explorer",
                                  name= "Auto_Display_Graphs_Explorer",
                                  datatype="GPBoolean",
                                  parameterType="Optional",
                                  direction="Input")
        param20.value = True
        param20.category = "Display Options"

        param21 = arcpy.Parameter(displayName="Workspace",
                                    name="Workspace",
                                    datatype="GPString",
                                    parameterType="Derived",
                                    direction="Output")
        
        param22 = arcpy.Parameter(displayName="Save Data in Excel File",
                                  name="Save_Data_as_EXCEL",
                                  datatype="GPBoolean",
                                  parameterType="Optional",
                                  direction="Input")
        param22.value = False
        param22.category = "Output Variable"
    ...
    def updateParameters(self, parameters):
        ...
        # Populate the discretization list
    ...
    def execute(self, parameters, messages):
        ...
        plot_hillslope = parameters[3].value
        ...
        plot_channel = parameters[9].value
        ...
        multiple_elements = parameters[18].value
        auto_display_graph = parameters[19].value
        auto_display_file = parameters[20].value
        save_data_as_excel = parameters[22].value
        ...
        if hillslope_ids is not None:
            agwa.plot_hydrograph(simulation, "Hillslope", hillslope_ids, multiple_elements, output_variable, 
                             simulation_directory, unit, auto_display_graph, save_data_as_excel)
            
        if channel_ids is not None:
            agwa.plot_hydrograph(simulation, "Channel", channel_ids, multiple_elements, output_variable, 
                             simulation_directory, unit, auto_display_graph, save_data_as_excel)
            
        if auto_display_file:
            subprocess.Popen(['explorer', os.path.join(simulation_directory, simulation)])
