...
def compare_hydrographs(simulation_list, hillslope_ids, channel_ids, output_variable, simulation_directory,
    ...
    # underscores for filenames; spaces in titles
    safe_variable_name = variable_name.replace(' ', '_')

    data_cache = {}

    def get_data(simulation, element_type, element_id):
        key = (simulation, element_type, element_id)
        if key not in data_cache:
            file_path = get_file_path(simulation_directory, simulation, element_type, element_id)
            data_cache[key] = read_simulation_data(file_path, unit)
        ...
        if ids is not None:
            for element_id in ids:
                ...
                for simulation in simulation_list:
                    df_sim_results = get_data(simulation, element_type, element_id)
                ...
                output_file_path = os.path.join(simulation_directory,
                                                 f"Compare_{safe_variable_name}_{element_type}ID_{element_id}_{unit}.png")
    ...
    if save_data_as_excel:
        excel_file_path = os.path.join(simulation_directory, f"Compare_{safe_variable_name}_{unit}.xlsx")
        ...
        with pd.ExcelWriter(excel_file_path, engine='openpyxl') as writer:
            for ids, element_type in [(hillslope_ids, "Hillslope"), (channel_ids, "Channel")]:
                if ids is not None:
                    for element_id in ids:
                        for simulation in simulation_list:
                            df_sim_results = get_data(simulation, element_type, element_id)[['tim_min', column_to_plot]]
                            raw_sheet = f"{simulation}_{element_type}ID_{element_id}"
                            df_sim_results.to_excel(writer, sheet_name=raw_sheet[:31], index=False)
